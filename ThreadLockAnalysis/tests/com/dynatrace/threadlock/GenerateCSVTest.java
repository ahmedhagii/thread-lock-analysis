...
class GenerateCSVTest extends TestClass {
    ...
    void shouldWriteDataToFile() throws IOException{
        ...
        generateCSV.openWriterBuffer("./tmp/test_file.csv");
        generateCSV.writeData(DummyData.CSV_DATA);
        generateCSV.closeWriterBuffer();

        BufferedReader reader = new BufferedReader(new FileReader("./tmp/test_file.csv"));
    ...
    void shouldReadDumpDirAndGenerateCSV() throws IOException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("./tmp/dump1.%s", DummyData.TIMESTAMP)));
        writer.write(DummyData.THREAD_DUMP);
        writer.flush();
        writer.close();

        writer = new BufferedWriter(new FileWriter(String.format("./tmp/dump2.%s", DummyData.TIMESTAMP)));
        writer.write(DummyData.THREAD_DUMP);
        writer.flush();
        writer.close();
        ...
        BufferedReader reader = new BufferedReader(new FileReader("./tmp/thread_data.csv"));
        List<String> returned = new ArrayList<>();
        while(reader.ready()) returned.add(reader.readLine());
